...
### [2026-07-06T23:24:52Z] 6-4-portal-polish-seats-left-states-empty-states-error-toasts — code-review: CLEAN, set done
- **Risk:** low
- **Workflow / step:** code-review step 03/04 (triage → resolve; fix-vs-action-items ask)
- **Decision point:** Fresh 3-lens adversarial review (Blind Hunter / Edge Case Hunter / Acceptance Auditor) of diff fc0e96c..HEAD (baseline_commit from story frontmatter) against story 6.4 ACs. Dev record NOT trusted — re-derived. At the fix-vs-action-items ask: default is FIX all HIGH/MEDIUM.
- **Options considered:** (A) done — clean, no HIGH/MEDIUM; (B) in-progress + Review Follow-ups for the one LOW; (C) auto-fix the LOW.
- **Chosen:** (A) done. Zero HIGH/MEDIUM/critical findings. One LOW dismissed with rationale; no code change this review.
- **Rationale:** Presentation-only consolidation, no domain/schema/guard/dep changes. Verified independently: prisma validate clean; next build "Compiled successfully" with all 10 portal/admin routes present+typed (ƒ Dynamic); vitest 507/507 green incl 8 new empty-state jsdom tests. AC1: browse grid AND detail page both render formatSeatsLeft(0)="Class full" badge + shared CLASS_FULL_MESSAGE constant (single source, replaces prior divergent copy). AC2: shared EmptyState Server Component adopted across all 7 portal/admin empty states; action is plain {href,label,accent?} data — RSC-500 safe (1.5 lesson), no Client element through non-children prop. AC3: all 5 toast islands surface toast.error via domain mappers with canonical "Something went wrong — please try again"; credit-wallet-form catch wording fixed em-dash. AC4: admin back links (classes/[id], students/[id]) gained focus-visible:ring-2 ring-ring; EmptyState CTA has min-h-[44px]+focus ring+tokens (no hex). AC5 green. Guards AD-3 (requireSession/requireAdmin still first), AD-5 (formatSeatsLeft sole label, no stored counter/write), AD-9 (formatZar at edge), AD-10 (no new meetingUrl/location surface) all confirmed untouched.
- **LOW dismissed:** my-classes empty "Browse available classes" changed from a subordinate underlined text-link to a default-variant (navy/primary token) filled Button via the shared EmptyState. Not a UX-DR2 violation: the default variant is navy — NOT a competing gold CTA — and the empty view has no other control, so "one gold CTA per group" holds. Making it a bespoke text-link would re-introduce per-page divergence the story exists to remove. No action.
- **Reversibility:** No code changes made this review; nothing to reverse. To reopen: set 6-4 back to in-progress in sprint-status.yaml and add Review Follow-ups.
- **Files touched:** none (review only); sprint-status.yaml status transition.
